...
export function BookCard({ title, author, coverUrl, categories, type, publishDate, status, onClick }: BookCardProps) {
  ...
  const statusColors = {
    Available: "bg-green-500/10 text-green-500 dark:bg-green-500/20",
    Borrowed: "bg-yellow-500/10 text-yellow-500 dark:bg-yellow-500/20",
    Reserved: "bg-blue-500/10 text-blue-500 dark:bg-blue-500/20",
  }

  return (
    <Card
      className="w-full max-w-[280px] bg-card border-border backdrop-blur-sm hover:bg-accent/50 transition-all duration-300 cursor-pointer"
      onClick={onClick}
    >
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-1">
            {categories.map((category) => (
              <Badge key={category} variant="secondary" className="bg-secondary/50 hover:bg-secondary">
                {category}
              </Badge>
            ))}
          </div>
